...
document.addEventListener('click', async (e) => {
	// If the clicked target is an anchor or child of an anchor, get this anchor element or return
	...
	// (Same path could be in-document-anchor and can be used to completely reload document from server)
	if (anchorParent.hostname !== window.location.hostname || anchorParent.pathname === window.location.pathname) return;
	e.preventDefault();
	...
	document.getElementsByTagName('body')[0].innerHTML = newdoc.getElementsByTagName('body')[0].innerHTML;
	document.getElementsByTagName('head')[0].innerHTML = newdoc.getElementsByTagName('head')[0].innerHTML;
